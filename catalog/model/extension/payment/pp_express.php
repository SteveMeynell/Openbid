...
class ModelExtensionPaymentPPExpress extends Model {
	...
	public function paymentRequestInfo() {
		...
		foreach ($this->cart->getFees() as $item) {
			$data['L_PAYMENTREQUEST_0_DESC' . $i] = '';

			$fee_count = 0;
			foreach ($this->cart->getFeeDetails($item['auction_id']) as $fee) {
				$value = $fee['amount'];
				$data['L_PAYMENTREQUEST_0_DESC' . $i] .= ($fee_count > 0 ? ', ' : '') . $fee['description'] . ':' . (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value);
				$fee_count++;
			}

			$data['L_PAYMENTREQUEST_0_DESC' . $i] = substr($data['L_PAYMENTREQUEST_0_DESC' . $i], 0, 126);

			$item_price = $this->currency->format($item['amount'], $this->session->data['currency'], false, false);

			$data['L_PAYMENTREQUEST_0_NAME' . $i] = $item['name'];
			$data['L_PAYMENTREQUEST_0_NUMBER' . $i] = '';
			$data['L_PAYMENTREQUEST_0_AMT' . $i] = $item_price;

			$item_total += number_format($item_price, 2, '.', '');

			$data['L_PAYMENTREQUEST_0_QTY' . $i] = '1';

			$data['L_PAYMENTREQUEST_0_ITEMURL' . $i] = $this->url->link('auction/auction', 'auction_id=' . $item['auction_id']);
