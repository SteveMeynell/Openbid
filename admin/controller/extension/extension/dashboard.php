...
class ControllerExtensionExtensionDashboard extends Controller {
	...
	public function MySort(&$mydata) {
		foreach ($mydata as $key => $row) {
		    $name[$key]  = $row['name'];
		    $width[$key] = $row['width'];
			$status[$key]	=	$row['status'];
			$sort_order[$key]  = $row['sort_order'];
		    $install[$key] = $row['install'];
			$uninstall[$key]	=	$row['uninstall'];
			$installed[$key] = $row['installed'];
			$edit[$key]	=	$row['edit'];
		}
		array_multisort($sort_order, SORT_ASC, $status, SORT_ASC, $mydata);
