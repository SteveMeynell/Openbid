...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			// Auctions
			$data['order_auctions'] = array();

			$auctions = $this->model_sale_order->getOrderAuctions($this->request->get['order_id']);

			foreach ($auctions as $auction) {
				$data['order_auctions'][] = array(
					'auction_id' => $auction['auction_id'],
					'name'       => $auction['name'],
					'fees'     => $this->model_sale_order->getOrderFees($this->request->get['order_id'], $auction['order_auction_id']),
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($auctions as $auction) {
				...
				$data['auctions'][] = array(
					'order_auction_id' => $auction['order_auction_id'],
					'auction_id'       => $auction['auction_id'],
					'name'    	 	   => $auction['name'],
					'fees'   		   => $fees,
					'num_fees'		   => $auction['num_fees'],
					'total'    		   => $this->currency->format($auction['total'], $order_info['currency_code'], $order_info['currency_value']),
					'href'     		   => $this->url->link('catalog/auction/edit', 'token=' . $this->session->data['token'] . '&auction_id=' . $auction['auction_id'], true)
				);
			}
	...
	public function invoice() {
		...
		$data['column_auction'] = $this->language->get('column_name');
		$data['column_fees'] = $this->language->get('column_fees');
		$data['column_num_fees'] = $this->language->get('column_num_fees');
		$data['column_total'] = $this->language->get('column_total');
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$auctions = $this->model_sale_order->getOrderAuctions($order_id);

				foreach ($auctions as $auction) {

					$fees = $this->model_sale_order->getOrderFees($order_id, $auction['order_auction_id']);
