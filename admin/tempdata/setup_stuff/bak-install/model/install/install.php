...
class ModelInstallInstall extends Model {
	public function database($data) {
		...
		$file = DIR_APPLICATION . 'openbid.sql';
		...
		if ($lines) {
			...
			foreach($lines as $line) {
				if ($line && (substr($line, 0, 2) != '--') && (substr($line, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$sql = str_replace("DROP TABLE IF EXISTS `ob_", "DROP TABLE IF EXISTS `" . $data['db_prefix'], $sql);
						$sql = str_replace("CREATE TABLE `ob_", "CREATE TABLE `" . $data['db_prefix'], $sql);
						$sql = str_replace("INSERT INTO `ob_", "INSERT INTO `" . $data['db_prefix'], $sql);
			...
			$db->query("INSERT INTO `" . $data['db_prefix'] . "user` SET user_id = '1', user_group_id = '1', username = '" . $db->escape($data['username']) . "', salt = '" . $db->escape($salt = token(9)) . "', password = '" . $db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', firstname = 'Your Name', lastname = 'Here', email = '" . $db->escape($data['email']) . "', status = '1', date_added = NOW()");
